...
class ReleaseValidator:
    ...
    def check_ada_source_headers(self) -> ValidationResult:
        ...
        # Filter to source directories only
        source_dirs = ["application", "bootstrap", "domain", "infrastructure", "presentation", "shared"]
    ...
    def check_guides(self) -> ValidationResult:
        ...
        for guide in required_guides:
            if not (guides_dir / guide).exists():
    ...
    def run_all_validations(self, skip_slow: bool = False) -> bool:
        ...
        if not skip_slow:
            ...
            self.print_warning("Skipping slow checks (build, tests) due to --quick flag")
